...
public static class DomainEventHandlersValidation
{
    ...
    public static IReadOnlyList<string> Validate(this DomainEventHandlers value)
    {
        ...
        var cacheField = fields.FirstOrDefault(f => f.Name == "_cacheService");
        var notificationField = fields.FirstOrDefault(f => f.Name == "_notificationService");
    ...
    /// <returns><c>true</c> when no validation problems are reported; otherwise <c>false</c>.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="value"/> is <c>null</c>.</exception>
