...
public sealed class ReviewServiceTests
{
    ...
    /// <summary>
    /// Tests that <see cref="ReviewService.CreateReviewAsync"/> updates an existing review when user already has a review for the product.
    /// This implements idempotent review submission - duplicate submissions update the existing review.
    /// </summary>
    [Fact]
    public async Task CreateReviewAsync_WithDuplicateReview_UpdatesExistingReview()
    {
        // Arrange
        var productId = 1;
        var userId = 5;
        ...
        _mockReviewRepository.Setup(r => r.SaveChangesAsync()).ReturnsAsync(1);
        _mockReviewRepository.Setup(r => r.FindAsync(It.IsAny<Expression<Func<Review, bool>>>())).ReturnsAsync(new List<Review> { existingReview });
        _mockProductRepository.Setup(r => r.Update(It.IsAny<Product>()));
        _mockProductRepository.Setup(r => r.SaveChangesAsync()).ReturnsAsync(1);

        // Act
        var result = await _reviewService.CreateReviewAsync(productId, userId, 5, "Updated Title", "Updated content");
        ...
        _mockReviewRepository.Verify(r => r.Update(existingReview), Times.Once);
